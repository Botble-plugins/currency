...
use Botble\Currency\Facades\CurrencyHelper as CurrencyFacade;
use Botble\Currency\Models\Currency;
use Botble\Currency\Supports\CurrencyHelper as CurrencySupport;
...
if (! function_exists('human_price_text')) {
    function human_price_text(float|null|string $price, Currency|null|string $currency, ?string $priceUnit = ''): string
    {
        ...
        if ($convertNumberToText = config('plugins.currency.general.display_big_money_in_million_billion')) {
        ...
        $space = (int) get_currency_setting('add_space_between_price_and_currency', 0) == 1 || $convertNumberToText ? ' ' : null;
...
if (! function_exists('format_price_currency')) {
    function format_price_currency(
        float|null|string $price,
        Currency|null|string $currency = null,
        bool $withoutCurrency = false,
        bool $useSymbol = true
...
if (! function_exists('get_currency_search')) {
    function get_currency_search($search)
    {
        return Currency::whereAny(['id', 'title', 'symbol'], $search)
                    ->first();
    }
}
